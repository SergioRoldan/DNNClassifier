...
#testing
def create_input_feat_fn(input_filename, batch_size, num_epoch=None, shuffle=True):
...
def get_features(input_filename, batch_size, num_epoch=None):
...
def train_DNN_classification_model(
    ...
    feat = get_features([train_tfrecord], 10000, num_epoch=1)
    ...
    height_numeric_column = tf.feature_column.numeric_column('insFrameHeight', shape=1)
    height_bucketized_column = tf.feature_column.bucketized_column(
        source_column = height_numeric_column,
        boundaries = bound_height
    )

    width_numeric_column = tf.feature_column.numeric_column('insFrameWidth', shape=1)
    ...
    insSymID_embbedinge_column = tf.feature_column.embedding_column(
    ...
    cm_data += '\nNot confused %: '+str(100*float(cm_correct)/float(cm_total))+'%'
